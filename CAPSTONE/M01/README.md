# BUS123 Capstone M01 Example Project

This folder contains a draft example capstone project for BUS123.

## Student-facing files

- `bus123-capstone-m01-l01-project.html`: printable project handout with the scenario, data packet, deliverables, and rubric.
- `bus123-capstone-m01-l01-starter.xlsx`: assumptions-only starter workbook for the Northstar Athletic Studio financing model. Students create the model tabs, rows, columns, formulas, formatting, charts, dashboard, and recommendation section themselves.

## Editable source

- `source/bus123-capstone-m01-l01-project.md`: editable source for the project handout content.

This example is not wired into `course-map.json` yet. Add it to the public course map only when it is ready to be released in the student sequence.
